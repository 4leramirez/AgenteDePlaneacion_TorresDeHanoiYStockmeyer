...
def goalDeArriba(goalMatrix, estadoActual, disco, mi_set):
...
def moverDisco(goalMatrix, estadoActual, disco, base):
...
#        print(str(i))
    ...
    if pivote < 0:
        ...
        if goalMatrix[0].size() == 0:
            ...
            goal = goalDeArriba(goalMatrix, estadoActual, pivote, mi_set)
            inPlace, distinto = checkDeArriba(estadoActual, goal, pivote)
            if not inPlace:
                set_Goal(goal, goalMatrix, distinto + 1)
                continue
    
    moverDisco(goalMatrix, estadoActual, pivote, base)
    ...
    for i in range(n_disks):
        print(str(matriz2[i][0]) + "  " + str(matriz2[i][1]) + "  " + str(matriz2[i][2]))
    print("_______")
#    print("Medio")
